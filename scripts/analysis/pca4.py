...
def load_and_prepare_data(tick_path, eigenval_path, metadata_path):
    ...
    # For Nebraska, still group by North/South if that column exists
    def assign_group(row):
        ...
        if row['State'] == 'Nebraska':
            # Check if North/South column exists and use it
            if 'North/South' in row and pd.notna(row['North/South']):
                return 'Nebraska North' if row['North/South'] == 'North' else 'Nebraska South'
            else:
                return 'Nebraska'
        # For all other states, just use the state name
        return row['State']
...
def plot_pca(pca_data, pve, pc1=1, pc2=2):
    ...
    # Dual vertical gradient with MORE distinct colors, including green on left and stronger red on right
    location_colors = {
        # LEFT SIDE VERTICAL GRADIENT (Cool colors - Purple, Blue, Green)
        # Top-left (Northeast): Deeper Purples and Blues
        'Maine': '#4B0082',            # Indigo
        'Wisconsin': '#00008B',        # Dark Blue
        'Michigan': '#1E90FF',         # Dodger Blue
        'Maryland': '#8A2BE2',         # Blue Violet (more purple for distinction)

        # Middle-left (Midwest/Transition): Blues and Greens
        'Iowa': '#00FFFF',             # Cyan
        'Minnesota': '#20B2AA',        # Light Sea Green (stronger green for distinction)

        # Bottom-left (Nebraska North): Distinct Green
        'Nebraska North': '#228B22',   # Forest Green (clear green for Nebraska North)
        'Nebraska': '#228B22',         # Fallback for general Nebraska, same as North

        # RIGHT SIDE VERTICAL GRADIENT (Warm colors - Strong Reds, Oranges, Yellows)
        # Top-right (Deep South): Strong Reds
        'Florida': '#B22222',          # Firebrick (strong red)
        'Virginia': '#8B0000',         # Dark Red (stronger red)
        'North Carolina': '#DC143C',   # Crimson (distinct red)

        # Middle-right (Mid-South): Oranges and Yellows
        'South Carolina': '#FF8C00',   # Dark Orange
        'Alabama': '#FFA500',          # Orange
        'Tennessee': '#FFD700',        # Gold
        'Texas': '#FF4500',            # Orange Red (Distinct from Tennessee)

        # Bottom-right (Kansas, Nebraska South & Oklahoma): Distinct colors
        'Kansas': '#FFDAB9',           # PeachPuff (Light, distinct orange-pink for Kansas)
        'Nebraska South': '#FF69B4',   # Hot Pink (very distinct)
        'Oklahoma': '#7CFC00',         # Lawn Green (a very different green to stand out)

        # Fallback for any unassigned states
    ...
    # Get unique groups present in the data and sort them for consistent legend order
    ...
    for group in present_groups:
        ...
        if not group_data.empty:
            # Get color for this group, use 'Other' color if not defined
            color = location_colors.get(group, location_colors['Other'])
    ...
    # Position legend outside plot area if many states
    ...
    # Print summary of states and sample counts
